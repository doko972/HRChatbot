

<footer class="footer">
    <div class="footer-content">
        <!-- Brand -->
        <div class="footer-brand-section">
            <a href="{{ route('home') }}" class="footer-brand">
                <lottie-player
                    src="/animations/logo.json"
                    background="transparent"
                    speed="1"
                    style="width: 32px; height: 32px;"
                    loop
                    autoplay>
                </lottie-player>
                HR Chatbot
            </a>
            <p class="footer-description">
                Votre assistant IA intelligent. Toujours disponible, toujours prêt à vous aider.
            </p>
        </div>

        <!-- Links -->
        <div class="footer-links">
            <span class="footer-links-title">Navigation</span>
            <a href="{{ route('home') }}">Accueil</a>
            <a href="{{ route('home') }}#features">Fonctionnalités</a>
            @auth
                <a href="{{ route('download') }}">Télécharger</a>
            @else
                <a href="{{ route('register') }}">S'inscrire</a>
            @endauth
        </div>

        <!-- Social -->
        <div class="footer-social">
            <span class="footer-social-title">Suivez-nous</span>
            <div class="footer-social-links">
                <a href="https://www.linkedin.com/" target="_blank" rel="noopener noreferrer" class="social-link" aria-label="LinkedIn">
                    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                    </svg>
                </a>
            </div>
        </div>
    </div>

    <div class="footer-bottom">
        <p>&copy; {{ date('Y') }} HR Chatbot. Tous droits réservés.</p>
    </div>
</footer>
